...
const cases = [
  ...
  { href: 'cases/mediapipe-magic-ball/', visual: true, zh: ['MediaPipe 魔法球', 'AI 视觉交互', '用实时手势与动态视觉，验证互动大屏和品牌活动的参与体验。', ['MediaPipe', 'JavaScript', 'Canvas']], en: ['MediaPipe Magic Ball', 'AI Visual Interaction', 'Live gesture-driven visuals for interactive displays.', ['MediaPipe', 'JavaScript', 'Canvas']] }
];
...
function renderCases() {
  ...
  grid.innerHTML = cases.map((item) => {
    ...
    return `<article class="case-card" data-case-link="${item.href}" tabindex="0" role="link" aria-label="${title}"><div class="case-top"><span class="case-category">${category}</span><span class="status ${item.live ? 'available' : 'in-progress'}">${status}</span></div><h3>${title}</h3><p>${summary}</p><div class="tags">${tags.map((tag) => `<span class="tag">${tag}</span>`).join('')}</div><div class="case-actions"><span class="case-view-hint">${window.t(item.visual ? 'card.view' : 'card.detail')} →</span></div></article>`;
  }).join('');
